...
class ExpressStatic
{
	...
	/**
	 * Default mime types for every extension
	 * @var array
	 */
	...
	/**
	 * Regex to check if a url ends with a static file
	 * @var string
	 */
	const REGEX_FILE = '/\.(png|gif|jpg|jpeg|html|htm|css|less|scss|ttf|bmp|ico|csv|doc|pdf|pls|ppt|tif|tiff|eps|ejs|swf|midi|mid|eot|woff|otf|svg|svgz|webp|docx|xlsx|xls|pptx|ps|class|jar|woff2)$/i';
	...
	{
		// Correct wrong previous correction
	...
	private function parse($route, $index = false)
	{
		...
		if (substr($route, -1) != '/' && !$this->isFile($route))
		{
			$route .= '/';
		}
		...
		if ($index && substr($route, -1) == '/' && !$this->isFile($route))
		{
			$route .= 'index.html';
		}
	...
	/**
	 * Check if the url is a folder/path or a file
	 *
	 * @param string Route
	 * @return bool
	 */
	...
	/**
	 * Try to get mime type from file ext
	 * If that fails, return default mime type
	 *
	 * @param string Route
	 * @return string
	 */
	...
	/**
	 * Normalize path with the correct OS directory separator
	 *
	 * @param string Path
